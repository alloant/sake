...
class FileHtml(object): 
    ...
    def icon_html_read_raw(self,reg):
        ...
        link = f'https://nas.prome.sg:5001/{chain}/{self.permanent_link}'
       
    
        #a2 = ET.Element('a',attrib={'class':'ms-1','href':f'/open_file?note={self.note_id}&reg={reg}&link={link}','target':'_blank'})
        ...
        #a2 = ET.Element('span',attrib={'hx-get':f'/open_file?note={self.note_id}&reg={reg}&file_clicked=True','hx-swap':'none'})
        #a.append(a2)
        i = ET.Element('i',attrib={f'class':f'bi {icon} position-relative','style':f'color: {color};'})
