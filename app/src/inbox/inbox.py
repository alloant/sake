...
def action_inbox_view(request):
    ...
    match action:
        ...
        case 'send_report_cg':
            ...
            notes = db.session.scalars(select(Note).where(or_(Note.has_target('cg'),and_(Note.reg.in_(['vcr','vc','cg','dg']),Note.flow=='out')),Note.sent_date>=sdate,Note.sent_date<=edate,Note.status=='sent'))
